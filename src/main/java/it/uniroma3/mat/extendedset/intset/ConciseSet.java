...
public class ConciseSet extends AbstractIntSet implements java.io.Serializable {
    ...
    public ConciseSet shiftLeft1Bit() {
        if (size() <= 0) {
            return empty();
        }

        ConciseSet answer = empty();
        answer.ensureCapacity(lastWordIndex + 1);

        WordIterator itor = new WordIterator();
  ...
  /*
    ...
    */
    ...
    private class LiteralAndZeroFillExpander implements WordExpander {
        ...
        @Override
        public WordExpander clone() {
            LiteralAndZeroFillExpander exp = new LiteralAndZeroFillExpander();
            System.arraycopy(buffer, 0, exp.buffer, 0, buffer.length);
            exp.current = current;
            exp.len = len;
            return exp;
        }
    ...
    private class OneFillExpander implements WordExpander {
        ...
        @Override
        public WordExpander clone() {
            OneFillExpander exp = new OneFillExpander();
            exp.firstInt = firstInt;
            exp.lastInt = lastInt;
            exp.current = current;
